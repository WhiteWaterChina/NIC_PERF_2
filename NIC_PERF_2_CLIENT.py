...
log_dir_prefix=log_dir_prefix + "/Stress/NIC_PERF_2"
...
for mtu_current in mtu_list:
    ...
    time.sleep(2)
    ssh_to_sut.connect(sut_ctrl_ip, 22, username=sut_username, password=sut_password)
    stdin_checkmtu, stdout_checkmtu, stderr_checkmtu = ssh_to_sut.exec_command(command="ip addr show|grep %s|grep mtu|awk '{match($0,/mtu\s*([0-9]*)/,a);print a[1]}'" % sut_devicename)
    ...
    #get sut test ip
    ssh_to_sut.connect(sut_ctrl_ip, 22, username=sut_username, password=sut_password)
    stdin_getip, stdout_getip, stderr_getip = ssh_to_sut.exec_command(command="ip addr show|grep %s|grep inet|awk '{match($s,/([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)/,a);print a[1]}'" % sut_devicename)
    ...
    kill_iperf = subprocess.Popen("killall -9 iperf3", shell=True, stdout=subprocess.PIPE)
sys.exit(0)